...
1. לחצתי על המקש בימני 
2. לחצתי חדש
3.layout file לחצתי על 
4. ובחרתי שם ואז לחתי ok
...
build.gradle (Project) — קובץ שמכיל הגדרות כלליות לכל הפרויקט, כמו plugins, repositories וכללים שמתקפים על כל המודולים.

build.gradle (Module) — קובץ שמכיל הגדרות ספציפיות לאפליקציה עצמה, כולל dependencies, גרסאות SDK, buildTypes והגדרות קומפילציה.



**למה חשוב להפריד בין קוד (java/) לבין משאבים (res/)?**

כדי לשמור על סדר בפרויקט
...
Retrofit היא ספרייה שמאפשרת לאפליקציה לבצע קריאות HTTP לשרתים בצורה מסודרת ונוחה.
היא מקלה על עבודה עם API, על המרת JSON לאובייקטים, ועל טיפול בבקשות בצורה אסינכרונית.
...
minSdk — הגרסה המינימלית של אנדרואיד שעליה האפליקציה יכולה לרוץ.

targetSdk — הגרסה שאליה האפליקציה מותאמת ומתוכננת.
לדוגמה: אם minSdk = 24, האפליקציה תרוץ על אנדרואיד 7 ומעלה. אם targetSdk = 36, האפליקציה מותאמת לגרסה החדשה ביותר של אנדרואיד.
...
**זמן שלקח לי:** לא יודע בארך 40 דקות
